...
/**
 * Conversation step in the two-step Q&A flow
 */
export type ConversationStep = 'initial' | 'follow_up';
...
/**
 * Initial response with follow-up questions (step 1)
 */
export interface SymptomCheckerInitialResponse {
  acknowledgment: string; // Brief acknowledgment of symptoms
  followUpQuestions: FollowUpQuestion[]; // 2-4 targeted questions
  preliminaryUrgency: UrgencyLevel; // Initial urgency assessment
  isEmergency: boolean; // If true, skip follow-up and show emergency
...
export interface SymptomCheckerRequest {
  ...
  // Two-step Q&A flow
  step?: ConversationStep; // 'initial' (default) or 'follow_up'
  followUpAnswers?: FollowUpAnswer[]; // Answers from step 1
  previousQueryId?: string; // Query ID from step 1

  // Legacy refinement (deprecated, use step instead)
...
export interface SymptomCheckerResponse {
  success: boolean;
  queryId: string;
  step: ConversationStep; // Which step this response is for
  // Step 1: Initial response with follow-up questions
  initialResponse?: SymptomCheckerInitialResponse;
  // Step 2: Final detailed response
