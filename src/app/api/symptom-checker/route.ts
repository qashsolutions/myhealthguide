...
1. This is NOT a medical diagnosis - you are providing general health information
2. Never claim to diagnose any condition
3. Always encourage seeking professional medical evaluation
4. Be compassionate and supportive in tone
5. For emergency symptoms, clearly state to call 911 or go to ER

Please provide a helpful response covering:

1. ASSESSMENT: A brief, caring overview of the described symptoms and general observations about what might be worth discussing with a doctor

2. POSSIBLE CAUSES: List 2-3 possible explanations (from common to less common) that a doctor might consider. Be clear these are possibilities, not diagnoses.

3. RECOMMENDED NEXT STEPS: Practical suggestions for what to do next (e.g., "Consider scheduling a doctor's visit", "Monitor for X days", etc.)

4. RED FLAGS TO WATCH: Specific warning signs that would warrant immediate medical attention

5. QUESTIONS FOR DOCTOR: 3-4 helpful questions to ask when they see their healthcare provider
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Build AI prompt
    const prompt = buildSymptomPrompt(body, userType === 'registered');
